...
var allowed = map[string]string{
	...
	"internal/mcodehelper/cli.go#newCLIHTTPClient":               "cli_artifacts：MiniMax Code 官方安装器透传",
	...
	// 有意直连或只走本机的例外。
	"internal/devhost/client.go#httpClient":          "主机上的守护进程 devd只经 SSH 转发通道可达（DialContext 走已登好的 SSH 连接），不经出站代理",
	"internal/devhost/client.go#streamClient":        "同上：创作工作空间搬运图像 / 视频的流式客户端，同一条 SSH 转发通道，不经出站代理",
	"internal/modeld/modeld.go#New":                  "模型服务节点守护进程 llmgate-modeld 自己的客户端：跑在被纳管主机上、不是设备进程，派发任务与探活直连内网算力服务器",
	"internal/modeld/cache.go#download":              "同上：拉取模型文件的无整体超时客户端，跑在被纳管主机上",
