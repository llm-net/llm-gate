...
// 自造的固定探测请求，验证地址、凭证、来源侧模型 ID
// 与账户余额这条链路。模型页「逐个测试所有来源」由前端按优先级串行调用本
// 端点实现——服务端保持单来源粒度，便于界面逐条给出进度与结果。
// 文本与 System One 模型支持探测；System One 用一次请求同时测试三种问题。
// 视频/图像来源的探测即真实提交付费任务，
...
type sourceTestResultJSON struct {
	Questions []upstream.SystemOneProbeQuestion `json:"questions,omitempty"`
...
func (s *Server) handleTestModelSource(w http.ResponseWriter, r *http.Request) {
	...
	acct := upstream.Account{
		...
		ProtocolURLs: cand.Upstream.ProtocolURLs,
		EgressMode:   cand.Upstream.EgressMode,
	}
	// 测试文本与语义判断来源：视频/图像入口的"最小探测"意味着真实提交一次付费
	// 生成任务，且提交/取消语义因厂商而异——那是任务适配器（迭代 8 后续
	// phase）的能力，不塞进这个 ping 探针。视频/图像来源得到空 results，审计
	// detail 记 no_protocol（与「该来源当前不服务任何入口」同一口径）。
	var protocols []string
	if route.Model.Kind == store.ModelKindText || route.Model.Kind == store.ModelKindSystemOne {
		doc, _ := s.effectivePlatformModels(r.Context())
		protocols = sourceProtocols(doc, route.Model.Kind, route.Model.Name, cand.UpstreamModelID, cand.Upstream.Type, cand.Upstream.CatalogID, cand.Upstream.BaseURL, cand.Upstream.ProtocolURLs)
	}
	results := make([]sourceTestResultJSON, 0, len(protocols))
	auditStatuses := make([]string, 0, len(protocols))
	// 转换来源的 Chat 与 Responses 共用一次 Chat 探测；原生 Responses 独立测试。
	probed := make(map[string]upstream.ProbeResult)
	for _, p := range protocols {
		wire := acct.WireProtocol(p)
		...
		results = append(results, sourceTestResultJSON{
			Protocol: p, OK: res.OK, Status: res.Status, LatencyMS: res.LatencyMS, Message: res.Message, Questions: res.Questions,
		})
		status := "net_error"
		if res.Status != 0 {
