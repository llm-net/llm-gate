...
//
// 骨架（`TopBarFrame`）、读数格（`Cell` / `CompactReading`）与配色菜单（`ThemeMenu`）
// 导出给 Key 持有人的「接入方法」页（features/access/holder-shell.tsx）复用：那一条
// 顶栏与这里同一副面孔，只是读数按 Key 裁剪、右端换成密钥标识与「退出」。
//
...
export type Reading = { tone: Tone; text: string; description: string; detail: string };

// 顶栏只显示模型数；数量单位与模型种类放到无障碍文本和悬停说明里。
export function cloudReading({ text, aigc, systemone = 0 }: api.APIModelCount): Reading {
  const total = text + aigc + systemone;
  const detail =
    systemone > 0
      ? t("文本模型 {text} 个 · 视频/图像模型 {aigc} 个 · 语义判断模型 {systemone} 个", { text, aigc, systemone })
      : aigc === 0
        ? t("文本模型 {text} 个", { text })
  ...
  return { tone: "ok", text: String(total), description: t("{n} 个模型", { n: total }), detail };
}

export function CompactReading({ label, reading }: { label: string; reading: Reading }) {
  return (
...
/** 顶栏上按钮的统一配色：白字压深底，悬停浅一档。 */
export const TOPBAR_BUTTON = "text-white/70 hover:bg-white/10 hover:text-white";

/**
 * 顶栏骨架：左端侧栏开关与品牌，中段读数条，右端操作钮。两种身份共用；`readings`
 * 是若干 `Cell`，`children` 是右端的按钮。
 *
 * `relative z-20`：侧栏那个容器是 `fixed z-10`，不抬一层的话顶栏的投影会被它盖住
 *（定位元素恒画在非定位元素之上）。
 */
export function TopBarFrame({ readings, children }: { readings: React.ReactNode; children: React.ReactNode }) {
  return (
    <header className="app-topbar relative z-20 flex shrink-0 items-center gap-3 px-3">
      <SidebarTrigger className={TOPBAR_BUTTON} />
      ...
      <div className="ml-auto flex h-7 min-w-0 items-center gap-0.5">{children}</div>
    </header>
  );
}

/** 配色圆点：点开是四种配色的单选菜单。首帧的真值由 index.html 的内联脚本挂到
 * <html> 上；这里的 state 只喂菜单的选中点（lib/theme.ts 的 useTheme，与登录页
 * 那条铭牌上的入口共用一份）。 */
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" title={t("界面配色")} aria-label={t("界面配色")}
          className={cn("size-7", TOPBAR_BUTTON)}>
...
export function TopBar() {
  ...
  return (
    <TopBarFrame readings={<Readings access={access} />}>
