...
func (a *app) planDerived(p *uninstallPlan, name string, purge bool) error {
	for _, file := range generatedNames(name) {
		...
		after := b
		switch {
		case name == "mcode" && (recorded || linkedProfile || p.state.Profiles[name]):
			// MCode rewrites JSON-as-YAML on model selection. The whole file
			// is generated by gate and contains a literal key; remove it even
			// after a rewrite, while keeping sessions and other profile data.
			after = nil
		case name == "grok":
...
func purgeNames(name string) []string {
	...
	case "opencode":
		return []string{"xdg"}
	case "mcode":
		return []string{"sessions", "v2", "logs", "cache", "plans", "subagents"}
	}
	return nil
}
...
func (a *app) planPrograms(p *uninstallPlan, name string) error {
	...
	for _, rel := range keys {
		...
		if name == "mcode" {
			allowed = filepath.Join("tools", "mcode", "releases") + string(filepath.Separator)
		}
